...
if ( ! class_exists( 'WWP_Admin_Custom_Fields_Variable_Product' ) ) {
    ...
    class WWP_Admin_Custom_Fields_Variable_Product {
        ...
        public function add_variation_custom_wholesale_bulk_action_options() {
            ...
            <optgroup label="<?php esc_attr_e( 'Wholesale', 'woocommerce-wholesale-prices-testing' ); ?>">

                <?php
                    foreach ( $all_wholesale_roles as $role_key => $role ) {
                        /* translators: %1$s: wholesale role name */
                        echo '<option value="' . esc_attr( $role_key ) . '_wholesale_price">' . esc_attr( sprintf( __( 'Set wholesale prices (%1$s)', 'woocommerce-wholesale-prices-testing' ), $role['roleName'] ) ) . '</option>';
                    }
            ...
            if ( WWP_ACS_Integration_Helper::aelia_currency_switcher_active() ) {
                ...
                <div class="wholesale-prices-options-group options-group" style="border-top: 1px solid #DDDDDD;">
                    ...
                    <header class="form-row form-row-full">
                        <h4 style="font-size: 14px; margin: 10px 0;"><?php esc_html_e( 'Wholesale Prices', 'woocommerce-wholesale-prices-testing' ); ?></h4>
                    ...
                    <div class="wholesale-price-per-role-and-country-accordion">
                        ...
                        foreach ( $all_wholesale_roles as $role_key => $role ) {
                            ...
                            <div class="section-container">
                                ...
                                $field_label = $woocommerce_currencies[ $base_currency ] . ' (' . $currency_symbol . ') <em><b>' . __( 'Base Currency', 'woocommerce-wholesale-prices-testing' ) . '</b></em>';

                                /* translators: %1$s: Wholesale role name,%2$s: currency name and symbol */
                                $field_desc = sprintf( __( 'Only applies to users with the role of %1$s for %2$s currency', 'woocommerce-wholesale-prices-testing' ), $role['roleName'], $woocommerce_currencies[ $base_currency ] . ' (' . $currency_symbol . ')' );
                                ...
                                foreach ( $wacs_enabled_currencies as $currency_code ) {
                                    ...
                                    /* translators: %1$s: Wholesale role name,%2$s: currency name and symbol */
                                    $field_desc = sprintf( __( 'Only applies to users with the role of %1$s for %2$s currency', 'woocommerce-wholesale-prices-testing' ), $role['roleName'], $woocommerce_currencies[ $currency_code ] . ' (' . $currency_symbol . ')' );
                                    ?>
        ...
        public function add_wholesale_sale_price_dummy_fields( $loop, $variation_data, $variation, $role, $role_key, $currency_symbol, $wholesale_price, $discount_type, $wholesale_percentage_discount ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter
            ...
            woocommerce_wp_text_input(
                array(
                    ...
                    'label'         => sprintf( __( 'Wholesale Sale Price (%1$s)', 'woocommerce-wholesale-prices-testing' ), $currency_symbol ) . ' <a href="#" class="wholesale_sale_schedule">' . esc_html__( 'Schedule', 'woocommerce-wholesale-prices-testing' ) . '</a><a href="#" class="cancel_wholesale_sale_schedule hidden">' . esc_html__( 'Cancel schedule', 'woocommerce-wholesale-prices-testing' ) . '</a>',
                    'placeholder'   => '',
                    'data_type'     => 'price',
                )
            );
            ...
            echo '<div class="form-field ' . esc_attr( $role_key ) . '_wholesale_sale_price_dates_fields wholesale_sale_price_dates_fields wholesale_sale_price_dates_fields__variations hidden">
                    <p class="form-row form-row-first">
                        <label>' . esc_html__( 'Sale start date', 'woocommerce-wholesale-prices-testing' ) . '</label>
                    ...
                    <p class="form-row form-row-last">
                        <label>' . esc_html__( 'Sale end date', 'woocommerce-wholesale-prices-testing' ) . '</label>
