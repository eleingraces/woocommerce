...
if ( ! class_exists( 'WWP_Dashboard' ) ) {
    ...
    class WWP_Dashboard {
         ...
         * @access private
         */
        const WWPP_PLUGIN_PATH = WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'woocommerce-wholesale-prices-testing-premium' . DIRECTORY_SEPARATOR . 'woocommerce-wholesale-prices-testing-premium.bootstrap.php';
            ...
            if ( function_exists( 'wc_admin_connect_page' ) ) {
                wc_admin_connect_page(
                    array(
                        ...
                        'title'     => __( 'Dashboard', 'woocommerce-wholesale-prices-testing' ),
                    )
                );
            }
        }
        ...
        public function permissions_check() {
            ...
            return new WP_Error( 'rest_cannot_view', __( 'Invalid Request.', 'woocommerce-wholesale-prices-testing' ), array( 'status' => rest_authorization_required_code() ) );
        }
        ...
        public function get_dashboard_data( $request ) {
            ...
            if ( isset( $activate_plugin ) ) {
                ...
                switch ( $activate_plugin ) {
                    case 'wwof':
                        $plugin_name = __( 'WooCommerce Wholesale Order Form plugin.', 'woocommerce-wholesale-prices-testing' );
                        break;
                    case 'wwlc':
                        $plugin_name = __( 'WooCommerce Wholesale Lead Capture plugin.', 'woocommerce-wholesale-prices-testing' );
                        break;
                    default:
                        $plugin_name = __( 'WooCommerce Wholesale Prices Premium plugin.', 'woocommerce-wholesale-prices-testing' );
                }

                if ( empty( $this->activate_plugin( $activate_plugin ) ) ) {
                    $response = array(
                        'status'  => 'success',
                        'message' => __( 'Successfully activated', 'woocommerce-wholesale-prices-testing' ) . ' ' . $plugin_name,
                    );
                } else {
                    $response = array(
                        'status'  => 'error',
                        'message' => __( 'Unable to activate', 'woocommerce-wholesale-prices-testing' ) . ' ' . $plugin_name,
                    );
                }
                ...
                $response = array(
                    ...
                    'filter_options'          => array(
                        'options' => array(
                            '30'     => __( 'Last 30 days', 'woocommerce-wholesale-prices-testing' ),
                            '14'     => __( 'Last 14 days', 'woocommerce-wholesale-prices-testing' ),
                            '7'      => __( 'Last 7 days', 'woocommerce-wholesale-prices-testing' ),
                            'year'   => __( 'Last 1 year', 'woocommerce-wholesale-prices-testing' ),
                            'custom' => __( 'Custom', 'woocommerce-wholesale-prices-testing' ),
                        ),
                        'default' => '30',
                    ),
                    'internationalization'    => $this->internationalization(),
                    ...
                    'wws_plugins'             => array(
                        'wwpp' => array(
                            'key'             => 'wwpp',
                            'name'            => __( 'Wholesale Prices Premium', 'woocommerce-wholesale-prices-testing' ),
                            'installed'       => WWP_Helper_Functions::is_wwpp_installed(),
                            'active'          => WWP_Helper_Functions::is_wwpp_active(),
                            'link'            => esc_url( WWP_Helper_Functions::get_utm_url( 'woocommerce-wholesale-prices-testing-premium', 'wwp', 'wwsdashboard', 'licenseboxwwpp' ) ),
                            'learn_more_link' => esc_url( WWP_Helper_Functions::get_utm_url( 'woocommerce-wholesale-prices-testing-premium', 'wwp', 'upsell', 'dashboardlearnmorewwpp' ) ),
                        ),
                        'wwof' => array(
                            'key'             => 'wwof',
                            'name'            => __( 'Wholesale Order Form', 'woocommerce-wholesale-prices-testing' ),
                        ...
                        'wwlc' => array(
                            'key'             => 'wwlc',
                            'name'            => __( 'Wholesale Lead Capture', 'woocommerce-wholesale-prices-testing' ),
        ...
        public function internationalization() {
            $texts = array(
                'dashboard'                 => __( 'Dashboard', 'woocommerce-wholesale-prices-testing' ),
                'quick_stats'               => __( 'Quick Stats:', 'woocommerce-wholesale-prices-testing' ),
                'quick_stats_note'          => __( 'The stats will only count orders that are in processing or completed status.', 'woocommerce-wholesale-prices-testing' ),
                'wholesale_orders'          => __( 'Wholesale Orders', 'woocommerce-wholesale-prices-testing' ),
                'wholesale_revenue'         => __( 'Wholesale Revenue', 'woocommerce-wholesale-prices-testing' ),
                'top_wholesale_customers'   => __( 'Top Wholesale Customers:', 'woocommerce-wholesale-prices-testing' ),
                'recent_wholesale_orders'   => __( 'Recent Wholesale Orders:', 'woocommerce-wholesale-prices-testing' ),
                'view_order'                => __( 'View Order', 'woocommerce-wholesale-prices-testing' ),
                'view_all_wholesale_orders' => __( 'View All Wholesale Orders &rarr;', 'woocommerce-wholesale-prices-testing' ),
                'helpful_resources'         => __( 'Helpful Resources:', 'woocommerce-wholesale-prices-testing' ),
                'getting_started_guides'    => __( 'Getting Started Guides', 'woocommerce-wholesale-prices-testing' ),
                'read_documentation'        => __( 'Read Documentation', 'woocommerce-wholesale-prices-testing' ),
                'settings'                  => __( 'Settings', 'woocommerce-wholesale-prices-testing' ),
                'contact_support'           => __( 'Contact Support', 'woocommerce-wholesale-prices-testing' ),
                'license_activation_status' => __( 'License Activation Status:', 'woocommerce-wholesale-prices-testing' ),
                'wholesale_prices_premium'  => __( 'Wholesale Prices Premium', 'woocommerce-wholesale-prices-testing' ),
                'wholesale_order_form'      => __( 'Wholesale Order Form', 'woocommerce-wholesale-prices-testing' ),
                'wholesale_lead_capture'    => __( 'Wholesale Lead Capture', 'woocommerce-wholesale-prices-testing' ),
                'view_licenses'             => __( 'View Licenses &rarr;', 'woocommerce-wholesale-prices-testing' ),
                'wholesale_suite_plugins'   => __( 'Wholesale Suite Plugins:', 'woocommerce-wholesale-prices-testing' ),
                'deactivated_plugins'       => __( 'Deactivated Plugins:', 'woocommerce-wholesale-prices-testing' ),
                'activate_plugin'           => __( 'Activate', 'woocommerce-wholesale-prices-testing' ),
                'click_to_activate'         => __( 'Click to activate the plugin.', 'woocommerce-wholesale-prices-testing' ),
                'no_data'                   => __( 'No Data', 'woocommerce-wholesale-prices-testing' ),
                'upgrade_now'               => __( 'Upgrade Now', 'woocommerce-wholesale-prices-testing' ),
                'links'                     => array(
                    'upgrade_now' => esc_url( WWP_Helper_Functions::get_utm_url( 'bundle', 'wwp', 'upsell', 'dashboardupgradenowbutton' ) ),
                ),
                'learn_more'                => __( 'Learn More &rarr;', 'woocommerce-wholesale-prices-testing' ),
            );
        ...
        public function get_wws_plugins_license_statuses() {
            ...
            foreach ( $wws_plugins_license_drm as $plugin_key => $plugin_data ) {
                if ( WWP_Helper_Functions::check_wws_plugin_min_version( strtolower( $plugin_key ), $plugin_data['version'] ) ) {
                    ...
                    $wws_license_statuses[ strtolower( $plugin_key ) ] = array(
                        'status' => 'inactive',
                        'text'   => $plugin_data['name'] . " (<span class='inactive'>" . __( 'Update Available', 'woocommerce-wholesale-prices-testing' ) . '</span>)',
                    );
                }
            }
