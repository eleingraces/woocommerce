    ...
    */
    ...
    public function add_schedules( $schedules = array() ) {
        ...
        $schedules['weekly'] = array(
            'interval' => 604800,
            'display'  => __( 'Once Weekly', 'woocommerce-wholesale-prices-testing' ),
        );
                ...
                if ( ! get_option( 'wwp_tracking_notice' ) || defined( 'WWS_TESTING_SITE' ) ) {
                    ...
                    if ( ! get_option( 'wwp_anonymous_data', false ) ) {
                        ...
                        if ( ! WWP_Helper_Functions::is_dev_url( network_site_url( '/' ) ) || defined( 'WWS_TESTING_SITE' ) ) {
                            ...
                            $output .= '<p style="font-weight:700;">' . __( 'Wholesale Suite Usage Tracking Permission', 'woocommerce-wholesale-prices-testing' ) . '</p>';
                            $output .= '<p>';
                            $output .= __( 'Allow Wholesale Suite to track plugin usage? Opt-in to let us track usage data so we know with which WordPress configurations, themes and plugins we should test with.', 'woocommerce-wholesale-prices-testing' );
                            $output .= '</p>';
                            $output .= '<a href="' . esc_url( $optin_url ) . '" class="button-primary">' . __( 'Allow', 'woocommerce-wholesale-prices-testing' ) . '</a>';
                            $output .= '&nbsp;<a href="' . esc_url( $optout_url ) . '" class="button-secondary">' . __( 'Do not allow', 'woocommerce-wholesale-prices-testing' ) . '</a>';
                            $output .= '</p></div>';
