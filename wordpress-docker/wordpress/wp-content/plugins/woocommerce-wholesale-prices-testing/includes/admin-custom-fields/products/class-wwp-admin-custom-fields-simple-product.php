    ...
    class WWP_Admin_Custom_Fields_Simple_Product {
        ...
        public function add_wholesale_price_fields_on_quick_edit_screen() {
            ...
            <div class="quick_edit_wholesale_prices" style="float: none; clear: both; display: block;">
                ...
                <h4><?php esc_html_e( 'Wholesale Price', 'woocommerce-wholesale-prices-testing' ); ?></h4>
                ...
                if ( WWP_ACS_Integration_Helper::aelia_currency_switcher_active() ) {
                    ...
                    */
                    ...
                    foreach ( $all_wholesale_roles as $roleKey => $role ) {
                        ...
                        $field_title = sprintf( __( '%1$s Price (%2$s)', 'woocommerce-wholesale-prices-testing' ), $role['roleName'], $currency_symbol );
        ...
        public function add_wholesale_price_fields( $product_type = 'simple' ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter
            ...
            if ( WWP_ACS_Integration_Helper::aelia_currency_switcher_active() ) {
                ...
                <div class="wholesale-prices-options-group options-group options_group hide_if_advanced_gift_card hide_if_external" style="border-top: 1px solid #EEEEEE;">
                    ...
                    <header>
                        <h3 style="padding-bottom: 10px;"><?php esc_html_e( 'Wholesale Prices', 'woocommerce-wholesale-prices-testing' ); ?></h3>
                    ...
                    <div class="wholesale-price-per-role-and-country-accordion">
                        ...
                        foreach ( $all_wholesale_roles as $role_key => $role ) {
                            ...
                            $field_label     = $wc_currencies[ $base_currency ] . ' (' . $currency_symbol . ') <em><b>' . __( 'Base Currency', 'woocommerce-wholesale-prices-testing' ) . '</b></em>';
                            /* translators: %1$s: Wholesale role name,%2$s: currency name and symbol */
                            $field_desc = sprintf( __( 'Only applies to users with the role of %1$s for %2$s currency', 'woocommerce-wholesale-prices-testing' ), $role['roleName'], $wc_currencies[ $base_currency ] . ' (' . $currency_symbol . ')' );
                            ?>
                            ...
                            <div class="section-container">
                                ...
                                foreach ( $wacs_enabled_currencies as $currency_code ) {
                                    ...
                                    /* translators: %1$s: Wholesale role name,%2$s: currency name and symbol */
                                    $field_desc = sprintf( __( 'Only applies to users with the role of %1$s for %2$s currency', 'woocommerce-wholesale-prices-testing' ), $role['roleName'], $wc_currencies[ $currency_code ] . ' (' . $currency_symbol . ')' );
                                    ...
                                    woocommerce_wp_text_input(
                                        array(
											...
											'class'       => $role_key . '_wholesale_price wholesale_price short',
											'label'       => $field_label,
											'placeholder' => __( 'Auto', 'woocommerce-wholesale-prices-testing' ),
											'desc_tip'    => 'true',
											'description' => $field_desc,
											'data_type'   => 'price',
            ...
            <?php
                ...
                // Check if WC Subscription is active.
                $wc_subscription_active = is_plugin_active( 'woocommerce-subscriptions/woocommerce-subscriptions.php' ) ? true : false;
                $wwpp_active            = is_plugin_active( 'woocommerce-wholesale-prices-testing-premium/woocommerce-wholesale-prices-testing-premium.bootstrap.php' ) ? true : false;
                ?>
                ...
                <div class="wholesale-prices-options-group options-group options_group hide_if_advanced_gift_card hide_if_external" style="border-top: 1px solid #EEEEEE;">
                    ...
                    <header>
                        <h3 style="padding-bottom: 10px;"><?php esc_html_e( 'Wholesale Prices', 'woocommerce-wholesale-prices-testing' ); ?></h3>
                        <p style="margin:0; padding:0 12px; line-height: 16px; font-style: italic; font-size: 13px;">
                            <?php esc_html_e( 'Set a wholesale price for this product.', 'woocommerce-wholesale-prices-testing' ); ?>
                            <?php echo $wwpp_active ? '' : '<a href="#" class="price-levels">' . esc_html__( 'Add additional wholesale price levels.', 'woocommerce-wholesale-prices-testing' ) . '</a>'; ?>
                        </p>
                    </header>
                    ...
                    foreach ( $all_wholesale_roles as $role_key => $role ) {
                        ...
                        $field_label = sprintf( __( 'Wholesale Price (%1$s)', 'woocommerce-wholesale-prices-testing' ), $currency_symbol );

                        /* translators: %1$s: Wholesale role name */
                        $field_desc       = sprintf( __( 'Wholesale price for %1$s customers', 'woocommerce-wholesale-prices-testing' ), str_replace( array( 'Customer', 'Customers' ), '', $role['roleName'] ) );
                        $field_desc_fixed = $field_desc;

                        /* translators: %1$s: Wholesale role name, %2$s: HTML tag (<br/>) */
                        $field_desc_percentage = sprintf( __( 'Wholesale price for %1$s customers %2$s Note: Prices are shown up to 6 decimal places but may be calculated and stored at higher precision.', 'woocommerce-wholesale-prices-testing' ), str_replace( array( 'Customer', 'Customers' ), '', $role['roleName'] ), '<br/>' );
                        ...
                        <div class="wholesale-prices-field wholesale-prices-field--simple">
                            ...
                            <div class="wholesale-prices-field-form-field-container">
                                ...
                                if ( empty( $WOOCS ) && empty( $woocommerce_wpml ) ) {
                                    woocommerce_wp_select(
                                        array(
											...
											'label'       => __( 'Discount Type', 'woocommerce-wholesale-prices-testing' ),
											'value'       => $discount_type,
											'options'     => array(
												'fixed' => __( 'Fixed', 'woocommerce-wholesale-prices-testing' ),
												'percentage' => __( 'Percentage', 'woocommerce-wholesale-prices-testing' ),
											),
											'desc_tip'    => 'true',
                                            /* translators: %1$s: HTML tag (<br/>) */
											'description' => sprintf( __( 'Choose Price Type%1$sFixed (default)%1$sPercentage', 'woocommerce-wholesale-prices-testing' ), '<br/>' ),
                                    ...
                                    woocommerce_wp_text_input(
                                        array(
											...
											'label'       => __( 'Discount (%)', 'woocommerce-wholesale-prices-testing' ),
											'placeholder' => '',
											'desc_tip'    => 'true',
											'description' => __( 'The percentage amount discounted from the regular price', 'woocommerce-wholesale-prices-testing' ),
											'data_type'   => 'price',
                                ...
                                if ( $wc_subscription_active && $wwpp_active ) {
                                    ...
                                    /* translators: %1$s: currency symbol */
                                    $field_signup_fee_label = sprintf( __( 'Wholesale Signup Fee (%1$s)', 'woocommerce-wholesale-prices-testing' ), $currency_symbol );
                                    /* translators: %1$s: Wholesale role name */
                                    $field_signup_fee_desc = sprintf( __( 'Wholesale Signup Fee for %1$s customers', 'woocommerce-wholesale-prices-testing' ), str_replace( array( 'Customer', 'Customers' ), '', $role['roleName'] ) );
                                    ...
                                    /* translators: %1$s: currency symbol */
                                    $field_signup_fee_discount_label = __( 'Signup Fee Discount (%)', 'woocommerce-wholesale-prices-testing' );
                                    /* translators: %1$s: Wholesale role name */
                                    $field_signup_fee_discount_desc = sprintf( __( 'Wholesale Signup Fee for %1$s customers', 'woocommerce-wholesale-prices-testing' ), str_replace( array( 'Customer', 'Customers' ), '', $role['roleName'] ) );
                                    ...
                                    woocommerce_wp_text_input(
                                        array(
											...
											'placeholder' => '',
											'desc_tip'    => 'true',
											'description' => __( 'The percentage amount discounted from the Sign-up fee', 'woocommerce-wholesale-prices-testing' ),
											'data_type'   => 'price',
        ...
        public function add_wholesale_sale_price_dummy_fields( $post_id, $role, $role_key, $currency_symbol, $wholesale_price, $discount_type, $wholesale_percentage_discount ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter
            ...
            if ( empty( $WOOCS ) && empty( $woocommerce_wpml ) ) {
                woocommerce_wp_text_input(
                    array(
						...
						'label'             => __( 'Sale Discount (%)', 'woocommerce-wholesale-prices-testing' ),
						'placeholder'       => '',
						'desc_tip'          => 'true',
						'description'       => __( 'The percentage amount discounted from the wholesale price', 'woocommerce-wholesale-prices-testing' ),
						'data_type'         => 'price',
						'custom_attributes' => array(
							'data-wholesale_role' => $role_key,
						),
            ...
            woocommerce_wp_text_input(
                array(
					...
					'label'       => sprintf( __( 'Wholesale Sale Price (%1$s)', 'woocommerce-wholesale-prices-testing' ), $currency_symbol ),
					'placeholder' => '',
					'description' => '<a href="#" class="wholesale_sale_schedule">' . __( 'Schedule', 'woocommerce-wholesale-prices-testing' ) . '</a>',
					'data_type'   => 'price',
                )
            );

            echo '<p class="form-field ' . esc_attr( $role_key ) . '_wholesale_sale_price_dates_fields wholesale_sale_price_dates_fields hidden">
				<label for="wholesale__sale_price_dates_from">' . esc_html__( 'Sale price dates', 'woocommerce-wholesale-prices-testing' ) . '</label>
				<input type="text" name="' . esc_attr( $role_key ) . '_wholesale_sale_price_dates_from" id="' . esc_attr( $role_key ) . '_wholesale_sale_price_dates_from" value="" placeholder="' . esc_html( _x( 'From&hellip;', 'placeholder', 'woocommerce-wholesale-prices-testing' ) ) . ' YYYY-MM-DD" maxlength="10" pattern="' . esc_attr( apply_filters( 'woocommerce_date_input_html_pattern', '[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])' ) ) . '" />
				...
				<a href="#" class="description cancel_wholesale_sale_schedule">' . esc_html__( 'Cancel', 'woocommerce-wholesale-prices-testing' ) . '</a>' . wp_kses_post( wc_help_tip( __( 'The sale will start at 00:00:00 of "From" date and end at 23:59:59 of "To" date.', 'woocommerce-wholesale-prices-testing' ) ) ) . '
			</p>';
        }
