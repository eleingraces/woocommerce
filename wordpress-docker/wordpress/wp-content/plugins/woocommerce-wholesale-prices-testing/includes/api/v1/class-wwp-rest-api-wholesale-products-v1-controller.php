...
class WWP_REST_Wholesale_Products_V1_Controller extends WC_REST_Products_Controller {
    ...
    public static function validate_wholesale_price( $request, $registered_wholesale_roles, $type = 'product' ) {
        ...
        if ( isset( $request['wholesale_price'] ) && is_array( $request['wholesale_price'] ) ) {
            ...
            if ( ! empty( $invalid_roles ) || ! empty( $invalid_wholesale_price ) ) {
                return new WP_Error(
                    ...
                    sprintf( __( 'Unable to %s. Invalid wholesale price.', 'woocommerce-wholesale-prices-testing' ), $method ),
                    array(
                        'status'                  => 400,
        ...
        // translators: %s: method name.
        return new WP_Error( 'wholesale_rest_' . $type . '_cannot_' . $method, sprintf( __( 'Unable to %s. Invalid wholesale price.', 'woocommerce-wholesale-prices-testing' ), $method ), array( 'status' => 400 ) );
    }
    ...
    public function get_items( $request ) {
        ...
        if ( ! empty( $wholesale_role ) && ! isset( $this->registered_wholesale_roles[ $wholesale_role ] ) ) {
            return new WP_Error( 'wholesale_rest_product_cannot_view', __( 'Invalid wholesale role.', 'woocommerce-wholesale-prices-testing' ), array( 'status' => 400 ) );
        }
    ...
    public function get_item( $request ) {
        ...
        if ( apply_filters( 'wwp_general_discount_is_set', false, $request ) === false && ( $only_return_wholesale_products || apply_filters( 'wwp_only_show_wholesale_products_to_wholesale_users', false, $request ) ) ) {
            ...
            if ( empty( $wholesale_role ) ) {
                return new WP_Error( 'wholesale_rest_product_cannot_view', __( 'Cannot view, please provide wholesale_role parameter.', 'woocommerce-wholesale-prices-testing' ), array( 'status' => 400 ) );
            }
            ...
            if (
                ...
                return new WP_Error( 'wholesale_rest_cannot_view', __( 'Not a wholesale product.', 'woocommerce-wholesale-prices-testing' ), array( 'status' => rest_authorization_required_code() ) );
            }
        }
    ...
    public function get_collection_params() {
        ...
        foreach ( $this->registered_wholesale_roles as $role => $data ) {
            $wholesale_price_properties[ $role ] = array(
                ...
                'description'       => sprintf( __( 'Wholesale price for %s', 'woocommerce-wholesale-prices-testing' ), $data['roleName'] ),
                'type'              => 'string',
        ...
        $params = array(
            'wholesale_price' => array(
                'description' => __( 'The product wholesale price.', 'woocommerce-wholesale-prices-testing' ),
                'type'        => 'object',
