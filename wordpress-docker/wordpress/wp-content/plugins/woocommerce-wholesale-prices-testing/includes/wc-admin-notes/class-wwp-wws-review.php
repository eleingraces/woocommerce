...
if ( ! class_exists( 'WWP_WWS_Review' ) ) {
        ...
        public function review_wws() {
            ...
            if ( get_option( WWP_REVIEW_REQUEST_RESPONSE ) === 'never-show' || get_option( WWP_REVIEW_REQUEST_RESPONSE ) === 'review-later' ) {
                ...
                try {
                    ...
                    $note->set_title( __( 'Review Wholesale Prices', 'woocommerce-wholesale-prices-testing' ) );
                    $note->set_content( $note_content );
                    $note->set_content_data( (object) array() );
                    $note->set_type( $note::E_WC_ADMIN_NOTE_INFORMATIONAL );
                    $note->set_name( self::NOTE_NAME );
                    $note->set_source( 'woocommerce-admin' );
                    $note->add_action( 'review-wws', __( 'Review Wholesale Prices', 'woocommerce-wholesale-prices-testing' ), $review_link, $note::E_WC_ADMIN_NOTE_ACTIONED, true );
                    $note->save();
