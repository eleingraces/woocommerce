...
if ( ! class_exists( 'WWP_Bootstrap' ) ) {
    ...
    class WWP_Bootstrap {
        ...
        private function _activate( $blog_id ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
            ...
            $wholesale_role_name = __( 'Wholesale Customer', 'woocommerce-wholesale-prices-testing' );
            ...
            if ( ! isset( $wholesale_roles['wholesale_customer'] ) ) {
                $this->_wwp_wholesale_roles->registerCustomRole(
                    ...
                    $wholesale_role_name,
                    array(
                        'desc' => __( 'This is the main wholesale user role.', 'woocommerce-wholesale-prices-testing' ),
                        'main' => true,
                    )
                );
            }
        ...
        public function new_mu_site_init( $blog_id ) {
            if ( is_plugin_active_for_network( 'woocommerce-wholesale-prices-testing/woocommerce-wholesale-prices-testing.plugin.php' ) ) {
        ...
        public function initialize() {
            ...
            if ( version_compare( get_option( 'wwp_option_installed_version', false ), $this->_wwp_current_version, '!=' ) || get_option( 'wwp_option_activation_code_triggered', false ) !== 'yes' ) {
                ...
                $network_wide = is_plugin_active_for_network( 'woocommerce-wholesale-prices-testing/woocommerce-wholesale-prices-testing.plugin.php' );
        ...
        public function add_plugin_listing_custom_action_links( $links, $file ) {
            if ( plugin_basename( WWP_PLUGIN_PATH . 'woocommerce-wholesale-prices-testing.bootstrap.php' ) === $file ) {

                $settings_link = '<a href="admin.php?page=wc-settings&tab=wwp_settings">' . __( 'Settings', 'woocommerce-wholesale-prices-testing' ) . '</a>';
                array_unshift( $links, $settings_link );

                if ( ! WWP_Helper_Functions::is_plugin_active( 'woocommerce-wholesale-prices-testing-premium/woocommerce-wholesale-prices-testing-premium.bootstrap.php' ) ) {
                    $upgrade_link = '<a href="' . esc_url( WWP_Helper_Functions::get_utm_url( 'bundle', 'wwp', 'upsell', 'pluginpagebundlelink' ) ) . '" target="_blank"><b>' . esc_html__( 'Upgrade To Premium', 'woocommerce-wholesale-prices-testing' ) . '</b></a>';
                    array_unshift( $links, $upgrade_link );

                }

                $getting_started          = '<a href="' . esc_url( WWP_Helper_Functions::get_utm_url( 'kb/woocommerce-wholesale-prices-testing-free-plugin-getting-started-guide', 'wwp', 'kb', 'wwpgettingstarted' ) ) . '" target="_blank">' . __( 'Getting Started', 'woocommerce-wholesale-prices-testing' ) . '</a>';
            ...
            if ( current_user_can( 'manage_woocommerce' ) && get_option( 'wwp_admin_notice_getting_started_show' ) === 'yes' ) {
                ...
                if (
                    ...
                    <div class="updated notice wwp-getting-started">
                        ...
                        <p><?php esc_html_e( 'Thank you for choosing Wholesale Suite &#8211; the most complete wholesale solution for building wholesale sales into your existing WooCommerce driven store.', 'woocommerce-wholesale-prices-testing' ); ?>
                        <p><?php esc_html_e( 'The free WooCommerce Wholesale Prices plugin lets you set wholesale pricing for wholesale level customers. Would you like to find out how to drive it?', 'woocommerce-wholesale-prices-testing' ); ?>

                        <p><a href="<?php echo esc_url( WWP_Helper_Functions::get_utm_url( 'kb/woocommerce-wholesale-prices-testing-free-plugin-getting-started-guide', 'wwp', 'kb', 'wwpgettingstarted' ) ); ?>"
                                target="_blank">
                                <?php esc_html_e( 'Read the Getting Started guide', 'woocommerce-wholesale-prices-testing' ); ?>
                                <span class="dashicons dashicons-arrow-right-alt" style="margin-top: 5px"></span>
                            </a></p>
                        <button type="button" class="notice-dismiss"><span
                                class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'woocommerce-wholesale-prices-testing' ); ?></span></button>
                    </div>
                    <?php
        ...
        public function run() {
            ...
            register_activation_hook( WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'woocommerce-wholesale-prices-testing' . DIRECTORY_SEPARATOR . 'woocommerce-wholesale-prices-testing.bootstrap.php', array( $this, 'activate' ) );
            register_deactivation_hook( WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'woocommerce-wholesale-prices-testing' . DIRECTORY_SEPARATOR . 'woocommerce-wholesale-prices-testing.bootstrap.php', array( $this, 'deactivate' ) );
