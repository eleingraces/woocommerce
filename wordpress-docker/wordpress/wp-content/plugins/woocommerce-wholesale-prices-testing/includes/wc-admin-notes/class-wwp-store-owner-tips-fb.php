...
if ( ! class_exists( 'WWP_Store_Owner_Tips' ) ) {
    ...
    class WWP_Store_Owner_Tips {
        ...
        public function join_store_owner_tips_note() {
            ...
            try {
                ...
                $note->set_title( __( 'Join Store Owner Tips Facebook Group', 'woocommerce-wholesale-prices-testing' ) );
                $note->set_content( $note_content );
                $note->set_content_data( (object) array() );
                $note->set_type( $note::E_WC_ADMIN_NOTE_INFORMATIONAL );
                $note->set_name( self::NOTE_NAME );
                $note->set_source( 'woocommerce-admin' );
                $note->add_action( 'join-store-owner-tips', __( 'Join Store Owner Tips on Facebook', 'woocommerce-wholesale-prices-testing' ), $join_store_owner_tips, $note::E_WC_ADMIN_NOTE_ACTIONED, true );
                $note->save();
