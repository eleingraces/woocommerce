<?php
/**
 * Plugin Name:          WooCommerce Wholesale Prices For Testing
 * Requires Plugins:     woocommerce
 * Plugin URI:           https://wholesalesuiteplugin.com
 * Description:          WooCommerce Extension For Testing Purposes
 * Author:               Rymera Web Co
 * Version:              1.0.0
 * Author URI:           http://rymera.com.au/
 * Text Domain:          woocommerce-wholesale-prices-testing-for-testing
...
require_once 'woocommerce-wholesale-prices-testing.options.php';
...
if ( WWP_Helper_Functions::is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
    ...
    if ( WWP_Helper_Functions::is_plugin_active( 'woocommerce-wholesale-prices-testing-premium/woocommerce-wholesale-prices-testing-premium.bootstrap.php' ) ) {
        ...
        $wwpp_plugin_data = WWP_Helper_Functions::get_plugin_data( 'woocommerce-wholesale-prices-testing-premium/woocommerce-wholesale-prices-testing-premium.bootstrap.php' );
        ...
        if ( $wwpp_plugin_data && version_compare( $wwpp_plugin_data['Version'], '1.27', '<' ) ) {
            ...
            function wwp_missing_plugin_dependency_notice() {
                ...
                $wwpp_basename    = plugin_basename( WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'woocommerce-wholesale-prices-testing-premium' . DIRECTORY_SEPARATOR . 'woocommerce-wholesale-prices-testing-premium.bootstrap.php' );
                $wwpp_update_link = wp_nonce_url( 'update.php?action=upgrade-plugin&plugin=' . $wwpp_basename, 'upgrade-plugin_' . $wwpp_basename );?>

                <div class="error">
                    <h3><?php esc_html_e( 'Important Notice:', 'woocommerce-wholesale-prices-testing' ); ?></h3>
                    <p>
                        <?php
                            printf(
                                // translators: %1$s Minimum WWPP version required, %2$s Minimum WWP version required.
                                esc_html__( 'We have detected an outdated version of WooCommerce Wholesale Prices Premium. You require at least version %1$s for this version of WooCommerce Wholesale Prices (%2$s). Please update now.', 'woocommerce-wholesale-prices-testing' ),
                    ...
                    <p>
                        <?php if ( get_option( WWPP_LICENSE_ACTIVATED ) === 'no' || get_option( WWPP_LICENSE_ACTIVATED ) === false ) : ?>
                            <a style="padding:10px 20px; background:#d96208; display:flex; width:fit-content; font-size:16px; text-decoration:none; color:#fff; font-weight:700;" href="<?php echo esc_url( WWP_Helper_Functions::get_utm_url( 'my-account/downloads/', 'wwp', 'notice', 'incompatiblewwpp' ) ); ?>" target="_blank">
                                <?php esc_html_e( 'Download Latest Version', 'woocommerce-wholesale-prices-testing' ); ?>
                            </a>
                        <?php else : ?>
                            <a style="padding:10px 20px; background:#46BF93; display:flex; width:fit-content; font-size:16px; text-decoration:none; color:#fff; font-weight:700;" href="<?php echo esc_url( $wwpp_update_link ); ?>">
                                <?php esc_html_e( 'Update Now', 'woocommerce-wholesale-prices-testing' ); ?>
                            </a>
                        <?php endif; ?>
                    </p>
                </div>
    ...
    if ( $execute_wwp ) {
        ...
        require_once 'woocommerce-wholesale-prices-testing.plugin.php';
    ...
    function wwp_missing_plugin_dependency_notice() {
        ...
        $plugin_install_text = '<a href="' . wp_nonce_url( 'update.php?action=install-plugin&plugin=woocommerce', 'install-plugin_woocommerce' ) . '">' . __( 'Click here to install from WordPress.org repo &rarr;', 'woocommerce-wholesale-prices-testing' ) . '</a>';

        if ( file_exists( trailingslashit( WP_PLUGIN_DIR ) . plugin_basename( $plugin_base_path ) ) ) {
            $plugin_install_text = '<a href="' . wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . $plugin_base_path . '&amp;plugin_status=all&amp;s', 'activate-plugin_' . $plugin_base_path ) . '" title="' . __( 'Activate this plugin', 'woocommerce-wholesale-prices-testing' ) . '" class="edit">' . __( 'Click here to activate &rarr;', 'woocommerce-wholesale-prices-testing' ) . '</a>';
        }
        ?>
            ...
            <p>
                <?php
                    echo wp_kses_post(
                        ...
                        sprintf( __( '%1$sWooCommerce Wholesale Prices%2$s plugin missing dependency.%3$sPlease ensure you have the %4$sWooCommerce%5$s plugin installed and activated.%6$s', 'woocommerce-wholesale-prices-testing' ), '<b>', '</b>', '<br/><br/>', '<a href="https://wordpress.org/plugins/woocommerce/" target="_blank">', '</a>', '<br/>' )
                    );
                ?>
